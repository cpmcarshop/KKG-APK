...
public class MainActivity extends Activity {
    ...
    private void buildUi() {
        ...
        bg.setColor(Color.rgb(17, 24, 43));
        bg.setCornerRadius(dp(22));
        bg.setStroke(dp(1), Color.rgb(38, 52, 82));
    ...
    private void updateNav() {
        for (int i = 0; i < nav.getChildCount(); i++) {
            ...
            boolean sel = ids[i].equals(currentTab);

            GradientDrawable d = new GradientDrawable();
            d.setCornerRadius(dp(18));
            d.setColor(sel ? Color.rgb(23, 57, 74) : Color.TRANSPARENT);
            v.setBackground(d);
            v.setTextColor(sel ? Color.rgb(0, 210, 255) : Color.LTGRAY);
        }
    }
    ...
    private void setupWebView() {
        ...
        web.setBackgroundColor(Color.rgb(10, 12, 27));
        ...
        web.setWebViewClient(new WebViewClient() {
            ...
            public boolean shouldOverrideUrlLoading(
                ...
                if (host != null &&
                        (host.equals("cpmcarshop.github.io") ||
                         host.endsWith(".cpmcarshop.github.io"))) {
                    return false;
                }
    ...
    private void injectMobileUi() {
        String css =
                ...
                "header{padding:11px 14px!important;}" +
                ...
                "@keyframes androidPageIn{from{opacity:.25;transform:translateX(10px)}to{opacity:1;transform:none}}";

        String js =
                "(function(){" +
                "if(document.getElementById('android-mobile-style'))return;" +
                "var s=document.createElement('style');" +
                "s.id='android-mobile-style';" +
                "s.textContent=" + JSONObjectQuote(css) + ";" +
                "document.head.appendChild(s);" +
                "document.querySelectorAll('.tab-button').forEach(function(b){" +
                "b.addEventListener('click',function(){" +
                "var x=document.querySelector('.tab-content.active');" +
                "if(x){x.classList.remove('android-page-in');" +
                "void x.offsetWidth;" +
                "x.classList.add('android-page-in');}" +
                "});" +
                "});" +
                "})();";
